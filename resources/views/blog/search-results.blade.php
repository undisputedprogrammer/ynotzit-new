...
@fragment('search-result') --}}
    ...
    @if ($blogs != null)
        ...
        @foreach ($blogs as $blog)
            <a @click.prevent.stop="$dispatch('linkaction',{link: '/blog?title={{$blog['slug']}}', route: 'view-blog', fragment: 'page-content'})" class="  rounded-sm  " href="/blog?title={{$blog['slug']}}">
                <div class="flex items-center space-x-2 bg-gray-100 bg-opacity-100 opacity-100 hover:bg-yellow-100 py-1 px-1 rounded-sm">
                    ...
                    <p class="text-sm font-montmedium leading-[1px]"  >{{$blog['title']}}</p>
                </div>
            </a>
        @endforeach
